...
(* FILL IN HERE *)
Lemma quadruple_triple : forall (n:int),
    triple (quadruple n)
      \[]
      (fun r => \[r=4*n]).
Proof.
...
Lemma triple_transfer : forall (p : loc) (pv:int) (q : loc) (qv:int),
    triple (transfer p q)
      (p ~~> pv \* q ~~> qv)
      (fun _ => p ~~> (pv + qv) \* q ~~> 0).
Proof.
...
Lemma triple_transfer_aliased : forall (p:loc) (pv:int),
    triple (transfer p p)
      (p ~~> pv)
      (fun _ => p ~~> 0).
Proof.
...
Lemma triple_ref_greater_abstract : forall (p:loc) (n:int),
  triple (ref_greater p)
    (p ~~> n)
    (funloc q => \exists (m : int), p ~~> n \* q ~~> m \* \[ m > n ]).
Proof using.
...
Proof using.
  ...
  xapp.
  xif.
  - intros. repeat xapp; try math.
    xsimpl. math.
  - intro H1. xval. xsimpl. math.
Qed.
...
Proof using.
  ...
  xapp.
  xif; intros.
  - repeat xapp. math. xsimpl. rewrite max_r. rewrite max_r; math. math.
...
Proof using.
  ...
  repeat xapp.
  xif; intros.
  - repeat xapp; try math. xsimpl. math.
  - xval. xsimpl; math.
Qed.
(** [] *)
