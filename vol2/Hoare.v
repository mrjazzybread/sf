...
Theorem hoare_pre_false : forall (P Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  intros P Q c st st' H1 H2 H3.
  apply st in H3. contradiction.
Qed.
(** [] *)
...
Proof.
  ...
  split; inversion H1; subst; rewrite t_update_shadow; rewrite t_update_same.
  - apply H2.
  - reflexivity.
Qed.
(** [] *)
...
Definition swap_program : com := <{ Z:=Y; Y:=X; X:=Z }>.
...
Theorem swap_exercise :
  ...
  {{Y <= X}}.
Proof.
  apply hoare_seq with (Q:= (X <= Z)%assertion).
  - apply hoare_seq with (Q:=(Y <= Z)%assertion).
    + eapply hoare_consequence_pre.
      * apply hoare_asgn.
      * assn_auto.
    + eapply hoare_consequence_pre.
...
Proof.
  unfold hoare_triple.
  intros H.
  assert (A : (X !-> 1) =[X := 3; Y := X]=>(Y !-> 3; X !-> 3; X !-> 1)).
  { apply E_Seq with (X !-> 3; X !-> 1).
    + apply E_Asgn. reflexivity.
    + apply E_Asgn. reflexivity. }
  apply H with (n:=1) in A.
  + simpl in A.
    unfold t_update in A.
    rewrite String.eqb_refl in A.
    discriminate.
  + reflexivity.
Qed.
(** [] *)
