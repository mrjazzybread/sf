...
Proof.
  intros. split; intros; inversion H; subst;
    ...
    inversion H5; destruct (beval st b); try  discriminate; try reflexivity.
Qed.
(** [] *)
...
Proof.
  intros.
  assert (A: bequiv <{true}> <{true}>).
  { unfold bequiv. intros. reflexivity. }
  unfold bequiv in H. simpl in H.
  unfold cequiv;
  split; intros H1;
    [apply while_true_nonterm with b c st st' in H | apply while_true_nonterm with <{true}> <{skip}> st st' in A];
  unfold not in H;
  contradiction.
Qed.
(** [] *)
...
Proof.
  ...
  - inversion H2; subst.
    apply E_Seq with (c1:= c2) (st:=st'1) in H5; try assumption.
    apply E_Seq with (c1:= c1) (st:=st) in H5; try assumption.
  - inversion H5; subst.
    apply E_Seq with (c1:=c1) (st := st) in H3; try assumption.
    apply E_Seq with (c1:= <{c1;c2}>) (st := st) in H7; try assumption.
Qed.
(** [] *)
...
Proof.
  ...
  split.
  - intros H2.
    assert (A : st' = (X !-> aeval st' a; st')).
    { unfold aequiv in H. rewrite <- H. simpl. rewrite t_update_same. reflexivity. }
    rewrite A.
    inversion H2. subst.
    apply E_Asgn.
    reflexivity.
  - intros H2.
    inversion H2. subst.
    unfold aequiv in H. rewrite <- H. rewrite t_update_same. apply E_Skip.
Qed.
...
Proof.
  ...
  split; intros H3; inversion H3; subst;
  apply E_Seq with (st' := st'0);
  [apply H1 | apply H2 | apply H1 | apply H2]; assumption.  
Qed.
(** [] *)
...
Proof.
  ...
  split; intros H4; inversion H4; subst;
  [apply E_IfTrue | apply E_IfFalse | apply E_IfTrue | apply E_IfFalse];
    try rewrite <- H1; try assumption;
    try rewrite H1; try assumption;
    [apply H2 | apply H3 | apply H2 | apply H3];
  assumption.
Qed.
(** [] *)
...
Fixpoint optimize_0plus_aexp (a : aexp) : aexp.  Admitted.
...
Proof.
  intros.
  induction a;
  try ( simpl; inversion H; subst; apply IHa1 in H2; apply IHa2 in H3;
    rewrite H2; rewrite H3; reflexivity ).
  - reflexivity.
  - inversion H. subst. simpl. unfold t_update .
    apply String.eqb_neq in H1.
...
Proof.
  ...
  unfold cequiv in H.
  destruct H with st st as [H1 H2].
  assert (A : st =[skip]=> st).
  { apply E_Skip. }
  apply H2 in A.
  remember loop_never_stops.
  unfold loop in n.
  apply n in A. apply A.
Qed.
(** [] *)
...
(* Hint: You may want to use [t_update_permute] at some point,
     ...
     hypothesis. You can use [discriminate] to discharge *)
  left.
  unfold pXY.
  unfold pYX.
  split; intro H1;
  ...
  inversion H2; subst;
  inversion H5; subst;
    [apply E_Seq with ( Y!-> n0; st) | apply E_Seq with (X !-> n0; st)];
    try (apply E_Havoc);
...
Proof.
  ...
  unfold ptwice in H1.
  unfold pcopy in H1.
  destruct H1 with empty_st (Y !-> 0; X !-> 1) as [H2 H3].
  assert (A : empty_st =[havoc X; havoc Y]=> (Y !-> 0; X !-> 1)).
  ...
  apply equal_f with Y in H8.
  rewrite String.eqb_refl in H8.
  inversion H7.
  subst.
  unfold t_update in H9.
  apply equal_f with X in H9.
  rewrite String.eqb_refl in H9.
  assert (B: (Y =? X)%string = false).
  { apply String.eqb_neq. discriminate. }
  rewrite B in H9.
  rewrite H9 in H8.
  discriminate.
Qed.
...
Proof.
  ...
  induction H2; try discriminate; injection H3; intros; subst.
  + apply eqb_neq in H1. simpl in H. rewrite H1 in H. discriminate.
  + apply IHceval2.
  - inversion H2_. subst. inversion H7. subst. simpl. unfold t_update.
    rewrite String.eqb_refl. intro C. rewrite add_comm in C. discriminate.
  - reflexivity.
Qed.
...
Proof.
  split; intros; destruct (st X =? 0) eqn:E;
    try (apply eqb_neq in E);
    [| apply p1_may_diverge in H| | apply  p2_may_diverge in H];
    try (apply E);
    try contradiction;
    inversion H; subst; try (simpl in H2; rewrite E in H2; discriminate);
    [unfold p2 | unfold p1];
...
Proof.
  intro H1.
  unfold cequiv in H1.
  remember (Z !-> 1; X !->0; X !-> 1) as f_st eqn:E.
  assert (A : forall st,  st <> f_st -> ~ (X !-> 1) =[p4]=> st).
  { intros. intro contra. inversion contra. subst.
    inversion H3. subst.
    simpl in H6. inversion H6. subst. simpl in H. contradiction.}
  assert (B : (X !-> 1) =[p3]=> (Z !-> 2; X !->0; Z!->1; X !-> 1)).
  ...
  apply A in B.
  + apply B.
  + rewrite E. intro contra. apply equal_f with Z in contra.
    unfold t_update in contra.
    rewrite String.eqb_refl in contra. discriminate.
Qed.
(** [] *)
...
Proof.
  ...
  - intros.
    unfold p5 in H.
    remember   <{ while X <> 1 do
       havoc X
       end }> as loop.
    induction H; try discriminate; injection Heqloop; intros.
    + rewrite H1 in H. simpl in H. symmetry in H. apply negb_sym in H.
      simpl in H. remember st as st'.
      assert (A : forall s,  st =[p6]=> s -> st' =[p6]=> s).
      { intros. rewrite Heqst'. apply H2.}
      apply A.
      assert (B : st' = (X !-> 1; st)).
      { apply functional_extensionality. intros.
        unfold t_update.
        destruct (X =? x)%string eqn:E.
        + apply eqb_eq in H. apply String.eqb_eq in E. rewrite <- E. apply H.
        + rewrite Heqst'. reflexivity.
      }
      rewrite B. apply E_Asgn. reflexivity.
    + rewrite H2 in H0. inversion H0. subst.
      apply  IHceval2  in Heqloop.
      inversion Heqloop. subst. simpl in *.
  ...
  - intros.
    ...
    + apply eqb_eq in E.
      ...
      { apply functional_extensionality.
        ...
        destruct (X =? x)%string eqn:E2.
        + apply String.eqb_eq in E2. rewrite <- E2. apply E.
        + reflexivity. }
      rewrite A.
    ...
    + apply E_WhileTrue with (X !-> 1; st).
      * symmetry. apply negb_sym. apply E.
      * apply E_Havoc.
      * apply E_WhileFalse.
        reflexivity.
Qed.
...
Theorem swap_noninterfering_assignments: forall l1 l2 a1 a2,
  ...
  cequiv
    ...
    <{ l2 := a2; l1 := a1 }>.
Proof.
  split; intro H2; inversion H2; subst; inversion H5; subst; inversion H8; subst;
    rewrite aeval_weakening in H8; auto;
    rewrite aeval_weakening in H2; auto;
    rewrite aeval_weakening; auto;
    rewrite t_update_permute; auto;
    [apply E_Seq with (l2 !-> aeval st a2; st) | apply E_Seq with (l1 !-> aeval st a1; st)];
...
Definition c3 : com := <{ X:=2 }>.

Definition c4 : com := <{ X:=3 }>.

Theorem c3_c4_different : ~ capprox c3 c4 /\ ~ capprox c4 c3.
Proof.
  split; intro H1; unfold capprox in H1.
  + assert (A : empty_st =[c3]=> (X !-> 2)).
    { apply E_Asgn. reflexivity. }
    apply H1 in A.
    inversion A. subst.
    apply equal_f with X in H4. discriminate.
  + assert (A : empty_st =[c4]=> (X !-> 3)).
    { apply E_Asgn. reflexivity. }
    apply H1 in A.
    inversion A. subst.
    apply equal_f with X in H4. discriminate.
